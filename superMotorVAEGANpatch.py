...
class PatchGANDiscriminator(nn.Module):
    def __init__(self, img_size=256, input_channels=1, ndf=64, n_layers=4, norm_layer=nn.BatchNorm2d):
        """
        PatchGAN discriminator that classifies if image patches are real or fake
        
        Parameters:
        -----------
        img_size : int
            Size of input image (assumed square)
        input_channels : int
        ...
        # No need for normalization in the first layer
        sequence = [
        ...
        # Output 1 channel prediction map (not sigmoid - we'll use BCEWithLogitsLoss)
        sequence += [
    ...
    def forward(self, input):
        """
        Forward pass
        Returns a patch of predictions (not averaged)
        """
...
def train_supervised_vaegan_patchgan(vae_model, discriminator, train_loader, dataset, target_recon_img,
                          ...
                          checkpoint_path="supervised_model.pth", recon_path="outputs"):
    """
    Train the supervised VAE-GAN model with PatchGAN discriminator
    """
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, labels) in progress_bar:
            ...
            # Discriminator loss with weighting (PatchGAN version)
            d_loss = discriminator_loss_patchgan(d_real, d_fake_recon, d_fake_samples, recon_sample_weight)
            d_loss.backward()
            d_optimizer.step()
            ...
            # VAE-GAN loss with classification loss and scheduled TC weight (PatchGAN version)
            (loss, p_loss, kld_loss, mi_loss, tc_loss, dwkl_loss, 
            adv_loss, cls_loss, label_losses) = vae_gan_classification_loss_patchgan(
                recon_batch, data, mu, log_var, logits, labels, d_fake_recon, d_fake_samples,
                perceptual_loss_fn, perceptual_weight=args.perceptual_weight,
                kld_weight=current_kld_weight, adv_weight=adv_weight, cls_weight=cls_weight, 
                recon_sample_weight=recon_sample_weight,
                mi_weight=1.0, tc_weight=tc_weight, dwkl_weight=1.0)
...
def discriminator_loss_patchgan(d_real, d_fake_recon, d_fake_samples, recon_sample_weight=0.5):
    """
    PatchGAN discriminator loss that handles both reconstructions and samples
    with configurable weighting.
    
    Parameters:
    -----------
    d_real : torch.Tensor
        Discriminator output on real images (patch of predictions)
    d_fake_recon : torch.Tensor
        Discriminator output on reconstructed images
    d_fake_samples : torch.Tensor
        Discriminator output on sampled images
    recon_sample_weight : float
        Weight of reconstruction loss vs sampling loss (0-1)
    ...
    # For PatchGAN, we use tensors of ones and zeros of the same shape as the output
    real_target = torch.ones_like(d_real)
    fake_target = torch.zeros_like(d_fake_recon)
    
    # Add some label smoothing for more stable training
    real_target = real_target * 0.9  # Smooth the real labels to 0.9
    
    # Real loss
    real_loss = F.binary_cross_entropy_with_logits(d_real, real_target)
    
    # Fake losses - handle both reconstructions and samples
    fake_recon_loss = F.binary_cross_entropy_with_logits(d_fake_recon, fake_target)
    fake_samples_loss = F.binary_cross_entropy_with_logits(d_fake_samples, fake_target)
...
def main(args):
    ...
    # Create PatchGAN Discriminator (replace the old Discriminator)
    ...
    if args.train:
        ...
        losses = train_supervised_vaegan_patchgan(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--patch_downsample', type=int, default=4, help='Determines the receptive field size of each output element for discriminator')
