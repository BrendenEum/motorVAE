...
final_sales_counts = final_sales_counts[final_sales_counts$year>=2007 & final_sales_counts$year<=2025,]

# Clean up sales data for labels
final_sales_counts$age = final_sales_counts$year_of_sale - final_sales_counts$year
...
ind = (fs$year==2007 & fs$make=="acura" & fs$model=="rl") |
  (fs$year==2012 & fs$make=="dodge" & fs$model=="charger") |
  (fs$year==2013 & fs$make=="ford" & fs$model=="f-150") |
  (fs$year==2014 & fs$make=="bmw" & fs$model=="335") |
  (fs$year==2015 & fs$make=="hyundai" & fs$model=="sonata") |
  (fs$year==2016 & fs$make=="mercedes-benz" & fs$model=="s-class") |
  (fs$year==2018 & fs$make=="cadillac" & fs$model=="cts") |
  (fs$year==2017 & fs$make=="chevrolet" & fs$model=="bolt") |
